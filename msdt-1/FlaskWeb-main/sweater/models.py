...
class Category(db.Model):
    ...
    category_id = db.Column(db.Integer,
                            primary_key=True)  # Уникальный идентификатор категории
    description = db.Column(db.Text,
                            nullable=False)    # Описание категории
    complexity = db.Column(db.BigInteger,
                           nullable=False)     # Сложность категории
...
class Task(db.Model):
    ...
    task_id = db.Column(db.Integer,
                        primary_key=True)     # Уникальный идентификатор задачи
    name = db.Column(db.Text,
                     nullable=False)          # Название задачи
    description = db.Column(db.Text)          # Описание задачи
    start_date = db.Column(db.DateTime,
                           nullable=False)    # Дата начала задачи
    end_date = db.Column(db.DateTime)         # Дата окончания задачи
    deadline = db.Column(db.DateTime,
                         nullable=False)      # Дедлайн задачи
    status = db.Column(db.Text,
                       nullable=False)        # Статус задачи
...
class User(db.Model, UserMixin):
    ...
    login = db.Column(db.Text,
                      nullable=False,
                      unique=True)         # Логин пользователя
    password = db.Column(db.Text,
                         nullable=False)   # Хэшированный пароль
    role = db.Column(db.Text,
                     nullable=False)       # Роль пользователя (например, работник или модератор)
    ...
    def calculate_rating(self):
        ...
        completed_tasks = sum(
            task.task.category.complexity for task in self.users_tasks
...
class UsersTask(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('Users.user_id'),
                        primary_key=True)  # Ссылка на пользователя
    task_id = db.Column(db.Integer, db.ForeignKey('Task.task_id'),
                        primary_key=True)    # Ссылка на задачу

    # Связи с пользователем и задачей
    user = db.relationship('User', backref=db.backref('users_tasks',
                                                      lazy=True))
    task = db.relationship('Task', backref=db.backref('users_tasks',
                                                      lazy=True))
